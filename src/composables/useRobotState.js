...
// 模块级单例状态
const state = ref(defaultRobotState());
...
// 监听电池电压
bus.on('ros:voltage', (voltage) => {
    state.value.voltage = voltage;
    lastUpdate.value = Date.now();
});
// 监听充电状态
bus.on('ros:charging', (charging) => {
    state.value.charging = charging;
    lastUpdate.value = Date.now();
});
// 监听充电电流
bus.on('ros:charging_current', (current) => {
...
//# sourceMappingURL=useRobotState.js.map