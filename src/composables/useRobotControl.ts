...
// 模块级单例配置
const maxLinearSpeed = useLocalStorage<number>('wheeltec:maxLinearSpeed', 0.5)
const maxAngularSpeed = useLocalStorage<number>('wheeltec:maxAngularSpeed', 1.0)
const speedStep = useLocalStorage<number>('wheeltec:speedStep', 0.1)
const controlMode = useLocalStorage<'joystick' | 'dpad'>('wheeltec:controlMode', 'joystick')
...
// 定时器：D-pad 模式下持续发送指令
let dpadTimer: ReturnType<typeof setInterval> | null = null

export function useRobotControl() {
  function sendCmdVel(linearX: number, angularZ: number): void {
  ...
  function startDpadMove(linearX: number, angularZ: number): void {
    stopDpad()
    const clampedLinear = clamp(linearX, -maxLinearSpeed.value, maxLinearSpeed.value)
    const clampedAngular = clamp(angularZ, -maxAngularSpeed.value, maxAngularSpeed.value)
    currentLinear.value = clampedLinear
